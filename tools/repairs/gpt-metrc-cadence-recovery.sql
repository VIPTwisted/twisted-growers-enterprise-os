...
begin
  ...
  select count(*) into v_count from public.metrc_scan_schedule s
    join jsonb_populate_recordset(null::public.metrc_scan_schedule,v_after) e using(job_name)
  ...
  if v_count<>3 then raise exception 'Metrc cadence policy changed; inspect before recovery'; end if;
  select jobid into strict v_job from cron.job
    where jobname='metrc-dispatcher' and schedule='*/5 * * * *'
  ...
  update public.metrc_scan_schedule s set run_times=e.run_times
    from jsonb_populate_recordset(null::public.metrc_scan_schedule,v_before) e
  ...
  if not exists(select 1 from cron.job where jobid=v_job and schedule='*/15 * * * *') then
