-- TG Enterprise OS — 0003 Operations
-- CODE-002 Scheduling, CODE-003 Production, CODE-007 Sales & S&OP, CODE-008 Purchasing & Cash, CODE-009 Shipping & AR
...
create table sales_orders (
  ...
  kind text not null default 'firm',            -- firm | forecast
  ordered_on date not null default current_date,
  requested_ship_on date,
  promised_ship_on date,
  status text not null default 'open',          -- open | confirmed | picked | shipped | delivered | invoiced | paid | cancelled
...
-- Vincent's gate (blueprint CODE-003): no release without a RELEASED allocation.
...
create table schedule_assignments (
  ...
  unique (employee_id, work_date, shift)          -- the no-double-booking rule, enforced
);
...
create table time_entries (
  ...
  exception_code text,                            -- absence/pto/late codes
  approved_by uuid references employees(id),
...
-- Per-employee weekly payroll accrual (REQUIREMENT #1: derived from each person's own rate)
...
create table overhead_items (
  ...
  is_280e_cogs boolean not null default false,     -- IRC 280E classification per line
...
create table licenses (
  ...
  license_type text not null,                      -- MC / MP / MB / RMD / retail etc.
...
-- Only released finished goods ship (blueprint CODE-009) — and COA gate already guards RTS.
