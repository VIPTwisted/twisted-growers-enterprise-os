# Confirmed settings saves — approved, awaiting authenticated UI verification

Status: owner approved the exact production changes in chat on September 12. Migration 20260912195558 is applied; follow-up 20260912205727 bounds permission lock waits. Backend source and prior regression tests ship through PR219. Frontend remains unmerged: secure OS sign-in returned Invalid login credentials, so CLAUDE.md F2 authenticated UI verification has not passed. Do not merge the frontend until that check passes. No credential rotation.
...
- Align `nav_role_visibility.nrv_write` with the existing governed `f_caller_is_admin()` capability already used by page_permissions. This changes which authenticated roles can write menu visibility and was explicitly approved.
...
- Permission saves take SHARE ROW EXCLUSIVE locks on the two permission tables, with a 5-second lock timeout. Migration application also uses a 30-second statement timeout. Contending edits can fail visibly and require retry. No business records or credentials are changed by migration application.

All four production tables have primary keys. Applied filenames and the migration-tree pin match the actual Supabase versions.
...
Signed-in browser verification is still outstanding because the available site tab is at Sign in. Use the secure browser authentication flow; do not request credentials in chat. The authenticated UI and realtime behavior must be inspected before deployment. The RPC definitions and permissions are applied and verified; no production permission edit has been submitted through the new UI.
