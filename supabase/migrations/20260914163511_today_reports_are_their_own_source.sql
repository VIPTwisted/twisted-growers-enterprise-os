-- BP-8 / BP-17-1: the first measurement showed a fresh reported defect ($0, age 0) ranking 272 of 313 under
-- severity × money × age — right, by the formula, and useless to the owner who asked for the bots' and
-- people's call-outs. Two changes, no new constants: (1) a report filed from a page (scope qa:, a defect) is
-- its own source, "report", so the strip counts it and a click shows it; (2) every source's top three are
-- always in the list (marked in_top=false when they sit outside the overall top 25), so no kind of decision
-- can be invisible behind the bulk families.
...
begin
  ...
  with fg as (
    select 'fg|' || f.severity || '|' || public.f_finding_family(f.headline) as key,
           case when bool_and(f.scope like 'qa:%') then 'report' else 'finding_group' end as source, f.severity,
...
-- the decision record accepts the new source name; f_decide keeps treating a report as a finding family (same key, same effect)
alter table public.decisions drop constraint if exists decisions_source_check;
alter table public.decisions add constraint decisions_source_check check (source in ('finding_group','report','issue_group','question','enhancement','correction','qa_enhancement'));
