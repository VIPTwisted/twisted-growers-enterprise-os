...
select
  ...
  residual_pct_of_wet,
  case
    when residual_lb < -tol_lb                                             then '1 IMPOSSIBLE'
    when wet_lb > 0 and residual_lb > wet_lb + tol_lb                      then '1 IMPOSSIBLE'
    when harvest_closed is not null
     and packaged_lb <= tol_lb and waste_lb <= tol_lb                      then '2 CLOSED WITH NOTHING TAKEN OFF'
    when harvest_closed is null
     and packaged_lb <= tol_lb
     and (current_date - harvest_started) > dry_window_days                then '3 OPEN, PAST THE DRY WINDOW, NOTHING TAKEN OFF'
  ...
  case
    when residual_lb < -tol_lb then
      'Metrc holds a NEGATIVE residual of ' || round(residual_lb,2) || ' lb. Packaged plus waste exceeds the wet weight that was entered. One of the three numbers is wrong in Metrc.'
    when wet_lb > 0 and residual_lb > wet_lb + tol_lb then
    ...
    when harvest_closed is not null and packaged_lb <= tol_lb and waste_lb <= tol_lb then
      'Closed in Metrc on ' || harvest_closed || ' with ' || round(wet_lb,1) || ' lb wet entered and nothing ever packaged and nothing ever wasted. The entire wet weight was written off as residual at finish.'
  ...
  case
    when residual_lb < -tol_lb or (wet_lb > 0 and residual_lb > wet_lb + tol_lb) then
  ...
  'The residual is DERIVED (wet minus waste minus packaged, plus restored). It is not a water measurement. Fresh-frozen harvests are excluded from the band tests because they are packaged wet and correctly show no moisture loss.'::text as measurement_caveat
from j
where
     residual_lb < -tol_lb
  or (wet_lb > 0 and residual_lb > wet_lb + tol_lb)
