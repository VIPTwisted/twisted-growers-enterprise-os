-- BP-12b-7 / rule J7: a room is never shown without its department. The calendar read carries
-- room_key (the short tag for lanes and chips) and room_qualified ("Cultivation · Flower Room #2")
-- for every rendered name; the page never prints a bare room.
...
declare v_from date := coalesce(p_from, date_trunc('month', current_date)::date - 60); v_to date := coalesce(p_to, date_trunc('month', current_date)::date + 120);
        v_rules jsonb; v_rooms jsonb; v_pulls jsonb; v_cult uuid; v_dept text;
begin
  if public.current_app_role() is null then raise exception 'Sign in.' using errcode = '42501'; end if;
  select id, name into v_cult, v_dept from hr.org_nodes where node_type = 'department' and lower(name) = 'cultivation' limit 1;
  v_dept := coalesce(v_dept, 'Cultivation');
  ...
  select jsonb_agg(jsonb_build_object('room_key', room_key, 'room_label', room_label, 'room_qualified', v_dept || ' · ' || coalesce(room_label, room_key), 'cycle_days', cycle_days, 'stagger_days', stagger_days, 'harvest_weekday', harvest_weekday,
  ...
  select jsonb_agg(jsonb_build_object(
      'id', p.id, 'pull_no', p.pull_no, 'room_key', p.flower_room, 'room_label', (select room_label from public.cult_cycle_policy c where c.room_key = p.flower_room),
      'room_qualified', v_dept || ' · ' || coalesce((select room_label from public.cult_cycle_policy c where c.room_key = p.flower_room), p.flower_room),
  ...
  return jsonb_build_object('from', v_from, 'to', v_to, 'as_of', now(), 'department', v_dept, 'rules', coalesce(v_rules, '{}'::jsonb), 'rooms', coalesce(v_rooms, '[]'::jsonb), 'pulls', coalesce(v_pulls, '[]'::jsonb),
