-- BUG: `if p_kind not in ('in','out')` never fires for NULL, because
-- NULL NOT IN (...) evaluates to NULL, not TRUE. So a null kind slipped past
-- the guard, failed the `= 'in'` test the same way, and fell through to the
-- clock-out branch — returning "No open punch to close" and creating nothing.
-- Every kiosk, badge and offline punch would have silently done nothing.
--
-- Two fixes: the guard now handles NULL explicitly, and NULL becomes a
-- deliberate feature — a wall terminal should not ask a gloved hand at 6:52
-- whether it is arriving or leaving. The system already knows: an open punch
-- means out, no open punch means in.
...
begin
  ...
  -- Infer direction when none is given. This is what a wall terminal wants:
  -- one button, and the system knows which way you are going.
...
comment on function public.f_punch is
  ...
  'out, none means in — because a wall terminal should not ask a gloved hand at '
  '6:52 whether it is arriving or leaving. Passing null used to fall through to '
  'clock-out and create nothing: NULL NOT IN (...) is NULL, not TRUE, so the '
  'guard never fired. Found by smoke test 8 Aug 2026 before a single real punch.';;
