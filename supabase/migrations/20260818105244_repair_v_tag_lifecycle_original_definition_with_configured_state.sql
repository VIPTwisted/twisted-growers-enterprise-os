/* Repair v_tag_lifecycle: the recursive patch is replaced by the original definition
 * with the state read from configuration.
 *
 * My previous migration tried to append a corrected Metrc link by selecting from the
 * view inside its own replacement — infinite recursion, and every query on the view
 * errored. The broken view was dropped under the E1 exception path (justified: it was
 * unreadable by anything) and is recreated here from its original definition with the
 * hardcoded Missouri URL replaced inline.
 *
 * metrc_screen now reads METRC_STATE from integration_secrets ('ma'), with a coalesce to
 * 'ma' so the link degrades to the correct state rather than erroring if RLS ever hides
 * the secrets table from the caller.
 */

create view public.v_tag_lifecycle as
with pkg as (
...
comment on view public.v_tag_lifecycle is
  ...
  'closed. Carries the COA document, the manifest document and the APEX INVOICE. Where a '
  'stage has not happened the note says why in words. metrc_screen reads METRC_STATE from '
  'integration_secrets — it was briefly hardcoded to Missouri, then briefly recursive; '
  'both mistakes are in the migration log. Owner instruction, 18 Aug 2026. Agent I.';
