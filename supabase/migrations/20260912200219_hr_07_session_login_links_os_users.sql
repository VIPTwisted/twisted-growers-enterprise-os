-- An OS login with no employee record (the owner's, today) still signs into HR as the person it is:
-- session_login creates/links the person from app_users on first entry, at the company node, with the
-- mapped role. Nothing is invented: name from app_users.display_name or the auth email.
  ...
  if v_person.id is null and auth.uid() is not null then
    ...
    if u.user_id is not null then
      if u.employee_id is not null then
        ...
        select coalesce(nullif(u.display_name, ''), split_part(au.email, '@', 1), 'OS user') into v_name from auth.users au where au.id = auth.uid();
