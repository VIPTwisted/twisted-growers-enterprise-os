-- BP-5-5 × BP-8: the draft's figure is the week's labour at the rates on file (estimated_cost on
-- v_schedule_draft_decisions — hours × tg_hourly_rate per line, placeholder rates included and counted), a draft is
-- critical inside the policy's post horizon (post_horizon_days), and while the onboarding step "first weekly schedule
-- posted" is open the draft carries the same Today weight as an onboarding blocker — it is that step's action. The two
-- People v1 drafts ranked 287 and 288 of 288 with $0 and age 0.
...
select d.id, d.title, d.covers_from, d.covers_to, d.department_id, dp.name as department, d.drafted_by_kind, d.agent_name, d.rationale, d.created_at,
       ...
       (select round(sum(extract(epoch from (l.planned_end - l.planned_start)) / 3600.0 * coalesce(r.rate, 0)), 2)
          from public.schedule_draft_lines l left join lateral (select rate, provisional from hr.tg_hourly_rate(l.employee_id, l.work_date)) r on true
         where l.draft_id = d.id and l.employee_id is not null) as estimated_cost,
       (select count(*) from public.schedule_draft_lines l join lateral (select rate, provisional from hr.tg_hourly_rate(l.employee_id, l.work_date)) r on true
         where l.draft_id = d.id and l.employee_id is not null and coalesce(r.provisional, true)) as lines_on_provisional_rates
...
begin
  ...
  with fg as (
    ...
    select 'sd|' || v.id, 'schedule_draft', case when v.covers_from <= current_date + coalesce((select post_horizon_days from public.scheduling_policy limit 1), 7) then 'critical' else 'elevated' end,
           ...
           'People v1 (' || v.drafted_by_kind || case when v.agent_name is not null then ' · ' || v.agent_name else '' end || ')', v.lines::int, coalesce(v.projected_cost_loaded, v.estimated_cost, 0), 0::numeric,
    ...
    select u.*, extract(day from now() - u.oldest)::int as age_days,
      (case u.severity when 'critical' then 4 when 'elevated' then 2 else 1 end)
        ...
        * (case when u.source = 'onboarding' or (u.source = 'schedule_draft' and exists (select 1 from public.v_onboarding_pack p where p.id = 'scheduling.first_week' and p.status <> 'done'))
                then greatest(coalesce(public.f_rule('today_onboarding_weight'), 1), 1) else 1 end) as score,
      case u.source
