 ...
 *    A missing relation errors, the client swallows it, and `out.goals = null` ships as
 *    context. The assistant has never once seen a target it was supposed to measure
 *    against, because a null read looks exactly like an empty table.
 *
 * 2. WRITES WERE OWNER/EXECUTIVE ONLY. The owner's rule is CEO, CFO and admins, plus
 *    anyone an admin later grants. Matches f_can_manage_inventory rather than inventing
 *    a second permission shape.
 *
 * 3. NO HARVEST OUTPUT TARGET EXISTED AT ALL.
 *
 * THE UNIT TRAP, RECORDED SO NOBODY RE-DERIVES IT WRONG. Owner, 11 Aug 2026: "TARGET IS
 * 2 HARVESTS A MONTH EACH 180 TOTAL 360", and 180 is POUNDS. But Metrc records 12-19
 * harvests a MONTH, because every room pull is its own harvest row - 14 rows carried
 * 368.5 packaged lb in Jul 2026, about 26 lb a row, not 180. The owner's "2 harvests" is
 * TWO FACILITY PULLS, not two Metrc harvest records, and harvest_plan_2026 agrees: it
 * keys on pull_no and facility_days_since_last_pull. Counting Metrc harvest rows against
 * a target of 2 would report the grow at seven times plan.
 *
 * The measurable, unambiguous target is therefore POUNDS PACKAGED PER MONTH, which needs
 * no interpretation and ties straight to v_monthly_conversion_truth.
 *
 * NOTHING HERE IS A BENCHMARK I INVENTED. Every seeded row carries its provenance and is
 * marked owner-stated. Actuals that cannot be computed honestly return NULL and say why -
 * never a plausible filler. grams_per_sqft is the case in point: the owner has ruled there
 * is NO square footage anywhere in this business and grow_rooms.sqft is null by design, so
 * its actual is structurally uncomputable and now says so instead of scoring the grow.
 *
 * direction is constrained to at_least | at_most | between - read from the live constraint,
 * not assumed.
 */
