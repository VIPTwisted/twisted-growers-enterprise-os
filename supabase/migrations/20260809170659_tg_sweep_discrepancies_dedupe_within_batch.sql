-- "ON CONFLICT DO UPDATE cannot affect row a second time": the same package
-- appears more than once on the same manifest in metrc_rpt_package_transfers, so
-- one discrepancy_key arrived twice in a single batch. Postgres refuses rather
-- than silently letting one row overwrite the other — which is the correct
-- behaviour and worth not working around blindly: the duplicate rows are real,
-- they just are not two separate disagreements.
--
-- DISTINCT ON collapses them to one, and the register counts a disagreement
-- once. Counting it twice is the same double-counting error that turned 687 lb
-- into 12,166 lb.
