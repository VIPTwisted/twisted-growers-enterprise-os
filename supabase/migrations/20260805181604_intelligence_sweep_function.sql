create or replace function tg_intelligence_sweep() returns table(new_findings int, open_total int) as $$
...
begin
  ...
  with found as (
    select 'Schedule discipline' as agent, 'critical' as severity,
      ...
      days_off_schedule as metric, 'days late' as units, null::numeric as dollars,
    ...
    select 'Loss and yield', case when waste_pct > 25 then 'critical' else 'elevated' end,
      ...
      waste_pct, 'percent waste', cost_of_waste, scope,
    ...
    select 'Room turnaround', 'critical', harvest || ' is blocking ' || room, drying_status,
      days_since_takedown, 'days since takedown', null, room,
    ...
    union all
    select 'Compliance watch', severity, flag || ' - ' || item, detail, quantity, uom, null,
    ...
    select 'Cash velocity', severity, 'Aging stock: ' || item,
      action || ' - ' || days_here || ' days in ' || location, days_here, 'days', null, location,
    ...
    select 'Allocation control', 'elevated', 'No approved allocation: ' || item,
      ...
      days_in_system, 'days in system', null, location,
    ...
    select 'Schedule discipline', 'elevated',
      ...
      coalesce(detail,'') || ' on ' || event_date, days_away, 'days away', null, coalesce(room,''),
  ...
  insert into agent_findings (agent, severity, headline, detail, metric, units, dollars, scope, action, drill_to, fingerprint)
  select d.agent, d.severity, d.headline, d.detail, d.metric, d.units, d.dollars, d.scope, d.action, d.drill_to, d.fingerprint
  from deduped d
...
select cron.unschedule(jobid) from cron.job where jobname = 'intelligence-sweep';
select cron.schedule('intelligence-sweep', '*/15 * * * *', $$ select tg_intelligence_sweep() $$);;
