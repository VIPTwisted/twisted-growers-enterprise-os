-- BP-6: two posting runs overlapped (the minute cron on top of the first manual run) and the second failed on the
-- journal's unique key. The engine now takes an advisory lock (an overlapping run returns 'skipped') and every
-- journal insert is ON CONFLICT DO NOTHING with the loop skipping what was already posted.
