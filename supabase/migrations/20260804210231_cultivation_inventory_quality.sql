-- TG Enterprise OS — 0002 Cultivation, Inventory, Quality (CODE-004/005/006 + REQUIREMENT #2)
...
create table coas (
  ...
  check (thc_pct is null or tac_pct is null or thc_pct <= tac_pct)
);
...
create table lots (
  ...
  source_type text not null default 'harvest',
  harvest_id uuid references harvests(id),
  cultivar_id uuid references cultivars(id),
  vendor_id uuid references vendors(id),
  metrc_tag text unique,
  batch_code text,
  ...
  note text,
  check (metrc_tag is null or length(metrc_tag) >= 16)
);
