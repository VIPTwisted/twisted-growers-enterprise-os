...
--
-- WHY: metrc-sync v19 states its own limitation - "a partial sweep does NOT resume.
-- Each run re-walks from page 1." With PAGE_SIZE 20 and a 110s soft deadline, the
-- plants/v2/inactive path (last of four) never reaches the end of the list, so whole
-- harvests are absent from the mirror. MEASURED before this change: 15,595 plants,
-- 290 of 380 harvests with ZERO plant records, earliest harvested plant 2025-12-15
-- against a first harvest of 2024-05-15.
--
-- PROVEN: a two-week window completes cleanly (status ok, 452 records, 37s). A
-- two-month window overruns (partial, 980 records, 152s). Two windows recovered
-- 1,364 plants and pushed the earliest harvested plant back to 2024-05-15.
--
-- This drives the EXISTING function through narrow windows. It changes no sync code.
-- v18 tried to fix resumption inside the function, was deployed unverified, and was
-- rolled back - so the resumption lives out here where it can be watched.
--
-- UNDO: select cron.unschedule('metrc-backfill');
--       drop function tg_metrc_backfill_next();
--       drop table metrc_backfill_window;
--       No existing table is altered and no row outside this table is written.
...
-- The two windows already run by hand today are done; do not repeat them.
update metrc_backfill_window
