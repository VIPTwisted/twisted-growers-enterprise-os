-- OWNER RULING, 26 August 2026, verbatim: "Moisture: 70-77% stays the official
-- expectation. Percentile cuts are extra context only, not the main flag."
--
-- WHAT WAS WRONG. The ladder I built ranked '4 OUTLIER AGAINST OUR OWN HARVESTS'
-- (the measured 5th-95th percentile cut) ABOVE '5 OUTSIDE THE OWNER-SET EXPECTED
-- BAND' (70-77). That made the agent-measured cut the primary flag and demoted
-- the owner's own band to a footnote - the exact inversion the ruling forbids.
--
-- WHAT CHANGES. One band tier now: outside the owner-set 70-77 band. The
-- percentile cut is demoted to a context column, outlier_context, appended at
-- the end. Nothing is removed and nothing is hidden: the same 201 harvests are
-- in the queue and the same 162 are outside the owner band. Only which of the
-- two bands does the flagging has changed.
--
-- The two conversion_factors rows stay because the context column reads them.
-- Their provenance is amended below to record that the owner has ruled they are
...
select
  ...
  case
    ...
    when weight_basis = 'dry' and harvest_closed is not null
     and (residual_pct_of_wet < owner_min_pct
       or residual_pct_of_wet > owner_max_pct)                             then '4 OUTSIDE THE OWNER-SET EXPECTED BAND'
  end as severity,
  case
    ...
    when weight_basis = 'dry' and harvest_closed is not null and residual_pct_of_wet < owner_min_pct then
      'Residual is ' || residual_pct_of_wet || '% of wet, below the owner-set expected band of ' || owner_min_pct || '% to ' || owner_max_pct || '%. More mass came off as packages than the water loss allows, so the packaged weight is carrying water.'
    when weight_basis = 'dry' and harvest_closed is not null then
      'Residual is ' || residual_pct_of_wet || '% of wet, above the owner-set expected band of ' || owner_min_pct || '% to ' || owner_max_pct || '%. ' || round(residual_lb,1) || ' lb was written off at finish.'
  ...
  'expected_moisture_pct_min / expected_moisture_pct_max (the flag) / dry_window_max_days'::text as rule_used,
  ...
  case
    when weight_basis <> 'dry' or harvest_closed is null then
      'Not applicable - the band tests only run on closed dried harvests.'
    when residual_pct_of_wet < outlier_min_pct then
      'CONTEXT ONLY, not the flag: this is also below ' || outlier_min_pct || '%, the 5th percentile of our own ' ||
      'closed dried harvests, so it is an outlier against our own record as well as against the owner band.'
    when residual_pct_of_wet > outlier_max_pct then
      'CONTEXT ONLY, not the flag: this is also above ' || outlier_max_pct || '%, the 95th percentile of our own ' ||
      'closed dried harvests, so it is an outlier against our own record as well as against the owner band.'
    when residual_pct_of_wet between owner_min_pct and owner_max_pct then
      'Inside the owner-set band, so it is not flagged at all.'
    else
      'CONTEXT ONLY, not the flag: outside the owner band but INSIDE our own measured spread of ' ||
      outlier_min_pct || '% to ' || outlier_max_pct || '%, which is where most of our harvests that miss the band sit.'
  end as outlier_context
from j
where
     residual_native < 0
...
'TICKET C2 QUEUE 1. Harvests whose Metrc residual (CurrentWeight) sits outside the owner-set expected band of 70-77 percent of wet weight, plus the harvests whose arithmetic is impossible or whose room has produced nothing. OWNER RULING 26 Aug 2026: the 70-77 band is the flag; the measured 5th-95th percentile cut is CONTEXT ONLY and appears in outlier_context, never in severity. Source metrc_harvests, corroborated against metrc_rpt_harvest_moisture. Sign tests run on native grams. Reports only - never corrects Metrc, never proposes a moisture band fix.';

-- The two percentile rows keep their measurement but record the owner's ruling
-- about what they are FOR, so nobody promotes them back to a threshold later.
update conversion_factors
   set label = 'Residual below this is an outlier - CONTEXT ONLY, not the flag',
       what_it_means = 'CONTEXT ONLY. Owner ruling 26 Aug 2026: the official expectation is the 70-77 percent band in expected_moisture_pct_min/max, and this percentile cut is extra context, not the main flag. It marks a dried harvest whose residual is below the bottom of our own measured spread, which is a stronger statement than missing the owner band but does not do the flagging.',
       evidence_note = 'Re-measurable at any time: percentile_cont over metrc_harvests where f_harvest_weight_basis(...) <> ''wet''. OWNER RULED 26 Aug 2026 that this is context only. It is read by v_xq_harvest_moisture.outlier_context and by nothing that decides severity.',
       set_by = 'Measured by Agent I (Claude), scope ruled by the owner 26 Aug 2026',
       updated_at = now()
 where key = 'harvest_residual_outlier_min_pct';

update conversion_factors
   set label = 'Residual above this is an outlier - CONTEXT ONLY, not the flag',
       what_it_means = 'CONTEXT ONLY. Owner ruling 26 Aug 2026: the official expectation is the 70-77 percent band in expected_moisture_pct_min/max, and this percentile cut is extra context, not the main flag. It marks a dried harvest whose residual is above the top of our own measured spread, which is a stronger statement than missing the owner band but does not do the flagging.',
       evidence_note = 'Re-measurable at any time. OWNER RULED 26 Aug 2026 that this is context only. It is read by v_xq_harvest_moisture.outlier_context and by nothing that decides severity.',
       set_by = 'Measured by Agent I (Claude), scope ruled by the owner 26 Aug 2026',
       updated_at = now()
 where key = 'harvest_residual_outlier_max_pct';

update open_questions
   set status = 'answered',
       answered_by = 'Owner (Vinny)',
       answered_at = now(),
       answer = 'Ruled 26 Aug 2026: "Moisture: 70-77% stays the official expectation. Percentile cuts are extra context only, not the main flag." The 70-77 band in expected_moisture_pct_min/max does the flagging in v_xq_harvest_moisture. The measured 46.2-86.1 percentile cut is reported per row in outlier_context and decides nothing.',
       last_seen = now()
 where question_key = 'harvest_residual_outlier_cuts';;
