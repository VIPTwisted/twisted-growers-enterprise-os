-- BP-8 × BP-13: an onboarding item on Today shows the step's own headline ("Every active employee has a kiosk PIN
-- (onboarding step, 0 of 27)"), not the number-collapsed family ("# of #") — one finding per step, so the family adds
-- nothing and hides the count. The decision key stays the family key (f_decide unchanged); members are looked up by
-- headline for this source.
