/* POSITIONS RESTATE AS OF THE SELECTED END DATE — the owner's ruling, in his
 * words: "all data must be relevant per date user selects! That's how all
 * platforms are built." He is right, and QuickBooks is the model he named:
 * inventory is shown AS OF the end of the range, derived from the ledger.
 * This mirror holds every birth (packaged_on), closure (Finished/Archived),
 * submission and result date — so the positions ARE derivable for any date,
 * and "cannot be restated" was underbuilding, not honesty.
 *
 * When the selected window ends before today, every Command position now
 * recomputes AS OF that end date from the ledger, with its quantity basis
 * stated on the tile (initial quantity for packages that later closed,
 * current for those still open — the mirror holds no per-day quantities).
 * When the window ends today, positions are today's, flows recompute for the
 * window, and the five window tiles (ords 91-95) tell the window's story.
 * Every tile moves with the dates. */
...
/* THE LEDGER, ONE ROW PER TAG, WITH ITS DATES — the base every as-of position reads. */
led as (
...
basis as (select '  ·  AS OF ' || (select t from w) || ', derived from the ledger (birth, closure, submission and result dates). Quantity basis: initial for packages that later closed, current for those still open — the mirror holds no per-day quantities.' note),
main as (
