...
do $$ begin
 if encode(sha256(convert_to(pg_get_functiondef('public.tg_apex_verification_finish(uuid,text,boolean,text,integer)'::regprocedure),'UTF8')),'hex') <> 'aeac78a36283448b8ba27ad1d06abb11f8acc2fd84a24168bee52de05479beaa' then raise exception 'Apex completion function changed since capture'; end if;
 if exists(select 1 from public.apex_sync_verification where state='running') then raise exception 'Drain active Apex syncs before changing completion policy'; end if;
end $$;
...
declare p public.apex_empty_history_proof%rowtype; e public.apex_entity%rowtype; ctx jsonb; request record; response record;
 ev jsonb:='{}'; body jsonb; data jsonb; reqs jsonb; params jsonb; path text; label text; key_value text; request_id bigint; scope text;
begin
 select entity into label from public.apex_empty_history_proof where id=p_id;
 select * into strict e from public.apex_entity where entity=label for update;
 ...
 begin
  ...
  if p.state='budget_pending' then
   ...
   select value into key_value from public.integration_secrets where name='APEX_API_KEY';
   reqs:=p.requests;
   foreach label in array array['company','welcome','default','not_cancelled','cancelled'] loop
    params:='{}';
    if label in ('company','welcome') then path:='/v1/'||label;
    else
     path:='/'||e.api_version||e.endpoint;
     params:=jsonb_build_object('page','1','per_page','1','updated_at_from',p.history_start::text);
     if label<>'default' then params:=params||jsonb_build_object('cancelled',case when label='cancelled' then 'true' else 'false' end); end if;
    end if;
    ...
    reqs:=reqs||jsonb_build_object(label,jsonb_build_object('id',request_id,'path',path,'params',params,'started_at',clock_timestamp()));
   end loop;
  ...
  foreach label in array array['default','not_cancelled','cancelled'] loop
   body:=(ev#>>array[label,'response_body'])::jsonb;
   params:=p.requests#>array[label,'params'];
   if jsonb_typeof(body->'orders') is distinct from 'array' or body->'orders'<>'[]'::jsonb
    ...
    raise exception 'Apex % response is not an explicit complete empty population',label;
   end if;
   if params->>'page' is distinct from '1' or params->>'per_page' is distinct from '1' or (params->>'updated_at_from')::timestamptz is distinct from p.history_start
    ...
    or (label='default' and params ? 'cancelled')
    or (label='cancelled' and params->>'cancelled' is distinct from 'true')
    or (label='not_cancelled' and params->>'cancelled' is distinct from 'false') then raise exception 'Unexpected source history filter'; end if;
  end loop;
