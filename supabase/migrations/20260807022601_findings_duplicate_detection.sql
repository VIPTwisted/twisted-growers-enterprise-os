/* DUPLICATE DETECTION
   -------------------
   The unified layer immediately showed two agents raising the same findings
   into different tables - identical dollars, identical pounds, same wording.
   Counted straight, open value read $7,113,370 when it is $4,456,760.

   Nothing is deleted. One row per duplicate set is canonical; the rest carry
   duplicate_of, so the drill-down still shows every copy and both agents keep
   working. Pages add "where is_duplicate = false" to get money that adds up.

   Conservative on purpose: two findings merge only when BOTH dollars and
   pounds match exactly AND the wording agrees. A finding with no figures is
   never merged - there is nothing solid to match it on.

   New columns are appended, not inserted: create-or-replace cannot reorder,
   and this view is never dropped. */
  ...
  select r.*,
    case when r.dollars is not null and r.pounds is not null
         then r.severity||'|'||r.dollars::text||'|'||r.pounds::text||'|'
              ||left(lower(regexp_replace(coalesce(r.what,''),'[^a-z ]','','gi')),24)
    end as merge_key
  from raw r
),
ranked as (
...
comment on view v_findings is
  'Every finding from every agent in one shape, duplicates marked. Filter is_duplicate = false for money that adds up. Additive - source tables unchanged.';
