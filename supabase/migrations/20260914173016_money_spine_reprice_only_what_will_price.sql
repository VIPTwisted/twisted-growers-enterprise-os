-- BP-6: the reprice section re-examined the same 2,000 oldest $0 postings every minute — the ones whose stream still has
-- no rate — and never reached the ones it could price. The candidate query now keeps only rows that will price.
