-- Clears strain discrepancies through the owner's naming ladder, recording on every row
-- WHICH RUNG answered and WHAT IT SAID, so any clearance can be re-derived and argued with.
--
-- On rule H1 (issues never clear themselves): these are not being cleared on an agent's
-- opinion. The owner ruled the precedence on 9 Aug 2026; each row carries the rung, the
-- evidence string and the tag, and f_strain_by_tag re-derives it on demand. Where the
-- ladder says Metrc itself is wrong, a correction is raised AT SOURCE (D2) and the local
-- clearance says so.
...
begin
  ---------------------------------------------------------------- blends
  with x as (
    select d.discrepancy_key, r.evidence, r.contributors
    from discrepancy_register d
    cross join lateral f_strain_by_tag(substring(d.subject from '1A[0-9A-Z]{22}')) r
    where d.resolved_at is null and d.class='strain' and r.verdict like 'BLEND%'
  )
  ...
  with x as (
    select d.discrepancy_key, r.strain, r.evidence
    from discrepancy_register d
    cross join lateral f_strain_by_tag(substring(d.subject from '1A[0-9A-Z]{22}')) r
    where d.resolved_at is null and d.class='strain'
      and r.strain is not null
      and lower(r.strain) = lower(btrim(d.source_b_says))
  )
  update discrepancy_register d
  set resolved_at = now(), resolved_by = p_by,
  ...
  for r in
    select btrim(regexp_replace(d.source_a_says,'^M\d+:\s*','')) as item_name,
           d.source_b_says as strain_field, s.strain, count(*)::int as packages
    from discrepancy_register d
    cross join lateral f_strain_by_tag(substring(d.subject from '1A[0-9A-Z]{22}')) s
    where d.resolved_at is null and d.class='strain'
      and s.strain is not null
      and lower(s.strain) = lower(btrim(regexp_replace(d.source_a_says,'^M\d+:\s*','')))
    group by 1,2,3
  loop
    if not exists (select 1 from metrc_corrections c
                   where c.title = 'Strain field wrong in Metrc: '||r.strain_field||' should be '||r.strain
      ...
      values
        ('Strain field wrong in Metrc: '||r.strain_field||' should be '||r.strain,
         r.packages||' packages carry the Metrc strain field "'||r.strain_field||'" while the '
           ||'source harvest they came from is "'||r.strain||'". Seed-to-sale is rung 1 of the '
           ||'naming ladder and outranks the strain field, so the strain field is the wrong one.',
         'Strain identity drives yield by strain, price by strain and the certificate chain. A '
           ||'package recorded under a strain it did not come from splits every per-strain figure '
           ||'across a name the plants never had. Correcting it only in our mirror would hide the '
           ||'fault from the state record (D2).',
         'In Metrc, correct the strain on these '||r.packages||' packages to "'||r.strain||'". '
           ||'Where a package is finished and cannot be edited, record the correction against the '
           ||'harvest and note that historic packages cannot be re-tagged.',
         r.packages::text, p_by,
         case when r.packages >= 50 then 'high' else 'normal' end,
         'select * from discrepancy_register d cross join lateral f_strain_by_tag('
           ||'substring(d.subject from ''1A[0-9A-Z]{22}'')) s where d.class=''strain'' and '
           ||'lower(s.strain) = lower('''||replace(r.strain,'''','''''')||''')');
      v_corr := v_corr + 1;
    end if;
  end loop;

  with x as (
    select d.discrepancy_key, s.strain, s.evidence
    from discrepancy_register d
    cross join lateral f_strain_by_tag(substring(d.subject from '1A[0-9A-Z]{22}')) s
    where d.resolved_at is null and d.class='strain'
      and s.strain is not null
  ...
  union all select 'left open for a person (ladder could not answer)',
