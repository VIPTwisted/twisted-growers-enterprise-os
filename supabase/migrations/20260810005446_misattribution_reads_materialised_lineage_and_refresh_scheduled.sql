...
with x as (
  ...
  left join mv_package_origin o on o.tag = p.tag
  where p.quantity > 0
)
select
  x.tag, x.item_name,
...
from x
left join metrc_documents d_coa on d_coa.package_tag = x.tag and d_coa.doc_type ilike '%coa%'
left join metrc_documents d_man on d_man.manifest_number = (x.g->'inbound_manifests'->>0) and d_man.doc_type ilike '%manifest%';

-- Lineage changes when packages are created or repackaged, which follows the
-- sync, not a page load. Refreshed CONCURRENTLY so readers are never blocked —
-- possible only because the matview carries a unique index.
select cron.unschedule('refresh-package-origin') where exists (select 1 from cron.job where jobname='refresh-package-origin');
select cron.schedule('refresh-package-origin', '18 * * * *',
                     $$refresh materialized view concurrently mv_package_origin$$);;
