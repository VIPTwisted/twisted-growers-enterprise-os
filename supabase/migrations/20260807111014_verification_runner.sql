/* The runner. Executes both derivations, compares, records, and raises a
   finding when they disagree - so a broken number reaches the CEO dashboard
   the same way a broken sync does.

   A check that ERRORS is treated as a failure, not skipped. A verification
   that cannot run proves nothing, and silence is the failure mode we are
   trying to eliminate. */
    ...
    if v <> 'agree' then
      insert into watchdog_findings
        (run_id, fingerprint, severity, what, where_it_is, who_is_accountable,
         why_it_matters, how_it_was_detected, what_to_do, the_arithmetic, dollars)
      values (v_run, 'verify:'||c.check_key,
        ...
        'Verification · '||c.check_key, c.owner,
        c.what_it_proves,
        'Derived the same fact two independent ways and compared them. '||c.source_a_label||' against '||c.source_b_label||'.',
        case when v='error'
          then 'The verification could not run: '||coalesce(msg,'?')||'. A check that cannot run proves nothing - fix the check before trusting the number.'
          else 'Find out which source is right before anyone quotes either figure. Do not average them and do not pick the convenient one.' end,
        case when v='error' then 'check errored: '||coalesce(msg,'?')
             else c.source_a_label||' = '||coalesce(a::text,'null')||' · '
                  ||c.source_b_label||' = '||coalesce(b::text,'null')||' · '
                  ||'apart by '||round(coalesce(pct,0),2)||'% (tolerance '||c.tolerance_pct||'%)' end,
        case when c.check_key like '%revenue%' or c.check_key like '%money%' then abs(diff) end)
...
comment on function tg_verify() is
  'Runs every verification check. Disagreement raises a finding. An errored check is a failure, never a skip.';;
